<script>
    ...
    import { idlFactory as backendIDL} from "../../declarations/vault/vault.did.js";
    ...
    onMount(async () => {
        ...
        if($auth.loggedIn) {
            ...
            backendActor = createCanisterActor(agent, backendIDL, process.env.VAULT_CANISTER_ID);
    ...
    async function depositT(principal) {
        ...
        //     await akitaActor.approve(Principal.fromText(process.env.VAULT_CANISTER_ID), depositAmount);
        ...
        //     await goldenActor.approve(Principal.fromText(process.env.VAULT_CANISTER_ID), depositAmount);
