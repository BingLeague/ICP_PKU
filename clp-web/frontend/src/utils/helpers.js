...
export const inputFun = (number) => {
    const maxValue = 9999999999.99
    if (String(number).includes(".")) {
        let numDian = number.toString().split(".")[1].length
        if (numDian > 6) {
            let dotIndex = number.indexOf(".")
            number = number.substring(0, dotIndex + 7)
        }
    }
    if (Number(number) > Number(maxValue)) {
        this.$toast("Over the top, please re-enter")
        number = ""
    }
    return number
}