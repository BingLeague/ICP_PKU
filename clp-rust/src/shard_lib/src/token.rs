...
impl Token {
    ...
    pub async fn fee(&self) -> Nat {
        let call_result: Result<(Nat,), _> =
            ic_cdk::api::call::call(self.principal, "icrc1_fee", ()).await;
        call_result.unwrap().0
    }

    pub async fn fee_balance(&self) -> Nat {
        let call_result: Result<(Nat,), _> =
            ic_cdk::api::call::call(self.principal, "fee_balance", ()).await;
        call_result.unwrap().0
    }

    // pub async fn allowance(&self, account: &Account, spender: &Account) -> Nat {
    //     let call_result: Result<(Allowance,), _> = ic_cdk::api::call::call(
    //         self.principal,
    //         "icrc2_allowance",
    //         (AllownArgs {
    ...
    //     )
    //     .await;
    //     call_result.unwrap().0.allowance
    // }
